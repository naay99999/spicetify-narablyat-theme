...
- `user.css` contains all visual styling, design tokens, glass effects, and Spotify selector overrides.
...
Use 2-space indentation in CSS and JavaScript. Keep CSS grouped by component or behavior, with short section comments matching the existing style. Prefer CSS custom properties in `:root` for colors, radii, transitions, and glass tokens instead of hardcoded repeated values.
